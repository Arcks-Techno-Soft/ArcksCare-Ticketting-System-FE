...
export default function InstallationFieldSignPage() {
  const params = useParams<{ token: string }>();
  const token = params?.token?.toString() ?? "";

  const [doc, setDoc] = useState<InstallationFieldDoc | null>(null);
  ...
  const fetchDoc = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/v1/sign/installations/${token}/field`);
      if (res.status === 404) {
      ...
      const data = (await res.json()) as InstallationFieldDoc;
      setDoc(data);
      setCustomerName(data.contact_name);
      setSubId(data.sub_engineers[0]?.id ?? null);
    } catch (e) {
      setLoadError(e instanceof Error ? e.message : "Failed to load installation document");
    }
  }, [token]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const updated = (await res.json()) as InstallationFieldDoc;
      setDoc(updated);
    } catch (e) {
    ...
    return (
      <Layout>
        <div className="mx-auto max-w-md py-24 text-center animate-rise-in">
          ...
          <p className="text-[12px] uppercase tracking-[0.18em] text-ink-subtle">Installation submitted</p>
          ...
          <p className="mt-4 text-[14.5px] leading-relaxed text-ink-muted">
            ...
            <strong>{fmtIst(doc.engineer_signed_at)}</strong>. The installation is now
            closed and the completion document is available to the SK-POS Care team.
  ...
  const addressLines = [doc.address_line1, doc.address_line2, doc.address_line3]
    .filter(Boolean)
    .join(", ");
  const cityLine = [doc.city, doc.state, doc.pincode].filter(Boolean).join(" ");
    ...
    <Layout>
      <section className="mx-auto max-w-3xl px-6 py-12">
        ...
        <p className="mt-4 max-w-xl text-[15.5px] leading-relaxed text-ink-muted">
          Review the installation below with the customer. Add photos of the
          completed setup, capture the customer&apos;s signature, then add your own
          to confirm the work is done. Submitting closes the installation.
        </p>
        ...
        <div className="mt-10 overflow-hidden rounded-xl2 border border-line bg-white shadow-soft">
          ...
          <div className="divide-y divide-line/60">
            <Section title="Customer">
              <Row k="Contact" v={doc.contact_name} />
              {addressLines && <Row k="Address" v={addressLines} />}
              {cityLine && <Row k="City" v={cityLine} />}
            </Section>

            <Section title="Installation">
              {doc.invoice_number && (
                <Row k="Invoice" v={<code className="font-mono text-[13px]">{doc.invoice_number}</code>} />
              )}
              {doc.engineer_name && <Row k="Engineer" v={doc.engineer_name} />}
              {doc.products_for_installation && (
                <div className="px-4 py-2.5">
                  <p className="text-[12px] uppercase tracking-[0.10em] text-ink-subtle">
                    Products installed
                  </p>
        ...
        <div className="mt-12 space-y-4">
          ...
          {doc.sub_engineers.length === 0 ? (
            <p className="rounded-xl2 border border-accent-danger/30 bg-white p-3.5 text-[13px] text-accent-danger">
              No sub-engineers are listed on this installation. Ask the SK-POS Care
              engineer to add you before signing.
            </p>
          ) : doc.sub_engineers.length === 1 ? (
            <>
              <p className="text-[13px] text-ink-muted">
                These details were entered for this installation by the SK-POS Care engineer.
              </p>
            ...
            <>
              <p className="text-[13px] text-ink-muted">
                ...
                installation by the SK-POS Care engineer.
              </p>
        ...
        <div className="mt-12 space-y-6">
          <div>
            ...
            <p className="mt-1 text-[13px] text-ink-muted">
              Add photos of the completed installation as a record of the on-site
              work. Short videos are accepted too.
            </p>
          </div>
        ...
        <div className="mt-12 space-y-6">
          <div>
            ...
            <p className="mt-1 text-[13px] text-ink-muted">
              Hand the device to the customer to confirm the installation is complete.
            </p>
          </div>
        ...
        <div className="mt-12 space-y-6">
          <div>
            ...
            <p className="mt-1 text-[13px] text-ink-muted">
              Sign below to confirm you completed the installation on site.
            </p>
          </div>
          ...
          <p className="text-[12px] text-ink-subtle">
            By submitting, both parties acknowledge the installation above is
            complete and working as expected. This closes the installation.
          </p>
        </div>
      </section>
    </Layout>
